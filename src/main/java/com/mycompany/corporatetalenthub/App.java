...
//        manager.agregarEmpleado(empleado);
    ...
    private static boolean registrarEmpleado(
        ...
        if (idEmpleado <= 0) {
            ...
            return false;
        } else if (idRepetido(empleados, posicion, idEmpleado)) {
        ...
        scanner.nextLine();
        empleados.add(new Empleado(idEmpleado, nombre, edad, salario));
    ...
    private static boolean idRepetido(
        ...
        for (var indice = 0; indice < cantidadEmpleados; indice++) {
            if (empleados.get(indice).getIdEmpleado() == idBuscado) {
                return true;
            }
        }
        return false;
